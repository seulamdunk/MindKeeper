	...
	<div class="col-md-8" style="width:300px;align:center;">
		<form action="/parkSearch" method="post" class="subscribe-form">
			...
			<div id="parkList" style="overflow:auto;width:300px;height:430px;padding:10px;">
				<c:forEach items="${parkSearchList }" var="parkSearchList">
				<div class="wrap">
					<div style="font-weight: bold;">${parkSearchList.parkName }</div>
			    	<div>
			    		<img class="parkScoreImg" src="../img/parkImg/parkScoreImg.png" alt="평점 이미지" width="18" height="18">${parkSearchList.parkScore } ,
			    		후기 : <a href="${parkSearchList.parkScoreLink }" target="_blank" class="parkLink" style="color:#007bff;">${parkSearchList.parkScoreCnt }</a> ,
			    	 	리뷰 : <a href="${parkSearchList.parkReviewLink }" target="_blank" class="parkLink" style="color:#007bff;">${parkSearchList.parkReview }</a>
			    	</div>
			     	<div>${parkSearchList.parkAddr }</div>
					<div><a href="${parkSearchList.parkLink }" target="_blank" class="parkLink" style="color:#007bff;">상세보기 ></a></div>
				</div>
				<hr>
				</c:forEach>
			</div>
